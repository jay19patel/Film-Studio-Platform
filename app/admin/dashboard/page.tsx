...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-gray-900 via-gray-900 to-gray-800 text-white rounded-3xl p-6 md:p-8 border border-gray-800 shadow-lg relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 rounded-full blur-2xl" />
        <h3 className="font-serif text-2xl font-bold tracking-tight mb-2">Hello, Studio Manager!</h3>
        <p className="text-gray-400 text-xs md:text-sm max-w-xl font-medium">
          Here is the summary of package inquiries and leads. You currently have <span className="text-amber-400 font-bold">{pendingLeads} new inquiries</span> waiting for action.
        </p>
      </div>

      {error && (
        <div className="bg-red-50 border border-red-150 text-red-700 text-xs p-4 rounded-2xl font-semibold">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* Pending Leads */}
        <div className="bg-white border border-gray-150 p-6 rounded-3xl shadow-xs flex items-center gap-4">
          <div className="bg-amber-50 text-amber-600 p-3.5 rounded-2xl border border-amber-100">
            <Clock className="h-6 w-6" />
          </div>
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">New Leads</span>
            <span className="text-2xl font-black text-gray-800">{pendingLeads}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-150 p-6 rounded-3xl shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Contacted</span>
            <span className="text-2xl font-black text-gray-800">{contactedLeads}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-150 p-6 rounded-3xl shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Completed</span>
            <span className="text-2xl font-black text-gray-800">{completedLeads}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-150 p-6 rounded-3xl shadow-xs flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Packages</span>
            <span className="text-2xl font-black text-gray-800">{totalPackages}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white border border-gray-100 rounded-3xl shadow-sm p-6 md:p-8">
          ...
          {inquiries.length === 0 ? (
            <div className="text-center py-12 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-sm text-gray-400 italic">No inquiries received yet.</p>
            </div>
          ) : (
                  ...
                  {inquiries.slice(0, 5).map((inq) => (
                    <tr key={inq.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="py-3.5 pr-3">
                        ...
                        <p className="text-gray-400 text-xs font-medium">{inq.phone}</p>
                      </td>
                      <td className="py-3.5 pr-3">
                      ...
                      <td className="py-3.5 text-gray-500 font-medium pr-3">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white border border-gray-100 rounded-3xl shadow-sm p-6 md:p-8">
            <h3 className="font-serif text-lg font-bold text-gray-900 mb-4">Quick Links</h3>
            <div className="space-y-3">
              <Link
                href="/admin/packages"
                className="flex items-center gap-3 p-3.5 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-amber-50/20 hover:border-amber-200 transition-all font-semibold text-sm text-gray-700"
              >
                <Package className="h-4.5 w-4.5 text-amber-500" />
                Manage Predefined Packages
              </Link>
              ...
              <Link
                ...
                className="flex items-center gap-3 p-3.5 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-amber-50/20 hover:border-amber-200 transition-all font-semibold text-sm text-gray-700"
              >
                <Gem className="h-4.5 w-4.5 text-amber-500" />
